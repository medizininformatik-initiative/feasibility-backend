...
public class DataqueryHandler {
  ...
  public ByteArrayOutputStream createCsvExportZipfile(Long dataqueryId, Principal principal) throws DataqueryException, IOException {
    ...
    if (dataquery.content().cohortDefinition().inclusionCriteria() == null) {
        ...
        files.put(dataquery.label() + "_inclusion_" + lang.getLanguage().toUpperCase() + ".csv", csvExportHandler.jsonToCsv(List.of(List.of()), lang));
      } else {
        files.put(dataquery.label() + "_inclusion_" + lang.getLanguage().toUpperCase() + ".csv", csvExportHandler.jsonToCsv(dataquery.content().cohortDefinition().inclusionCriteria(), lang));
      }
      if (dataquery.content().cohortDefinition().exclusionCriteria() == null) {
        // Call with empty lists to just write the headers to the file
        files.put(dataquery.label() + "_exclusion_" + lang.getLanguage().toUpperCase() + ".csv", csvExportHandler.jsonToCsv(List.of(List.of()), lang));
      } else {
        files.put(dataquery.label() + "_exclusion_" + lang.getLanguage().toUpperCase() + ".csv", csvExportHandler.jsonToCsv(dataquery.content().cohortDefinition().exclusionCriteria(), lang));
      }
      if (dataquery.content().dataExtraction() == null) {
        // Call with empty lists to just write the headers to the file
        files.put(dataquery.label() + "_features_" + lang.getLanguage().toUpperCase() + ".csv", csvExportHandler.jsonToCsv(DataExtraction.builder().build(), lang));
      } else {
        files.put(dataquery.label() + "_features_" + lang.getLanguage().toUpperCase() + ".csv", csvExportHandler.jsonToCsv(dataquery.content().dataExtraction(), lang));
      }
    }
